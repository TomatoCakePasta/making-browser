...
impl ComputedStyle {
    ...
    // If the value of each field in the ComputedStyle structure is not set, 
    // the default value is set.
    pub fn defaulting(&mut self, node: &Rc<RefCell<Node>>, parent_style: Option<ComputedStyle>) {
        // If a parent node exists and the parent's CSS 
        // and values ​​are different from the initial values, the values ​​are inherited.
    ...
    pub fn font_size(&self) -> FontSize {
        self.font_size
            .expect("failed to access CSS property: font_sizez")
    }
