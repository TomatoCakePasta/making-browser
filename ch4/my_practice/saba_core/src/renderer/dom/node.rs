...
// Even if the target disappears, it remains.
// Prevents circular references

// Window structure has root of a DOM tree
#[derive(Debug, Clone)]
pub struct Window {
    // document means root of a DOM tree
...
pub struct Node {
    // node types
    pub kind: NodeKind,
    // A window with a DOM tree
    // A page has one window instance
...
pub enum NodeKind {
    ...
    // and add new node to DOM tree using appendChild, insertBefore
    Document,
    
    // This interface is responsible for a content node in the DOM tree, 
    // such as a <p> tag.
    Element(Element),
